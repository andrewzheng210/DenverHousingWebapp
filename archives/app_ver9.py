from utils import load_files, draw_base_map, draw_house_base_map, update_house_base_map, predict_house_xgb, draw_family_base_map, write_about, family_demo_filter
...
json_neighbor, neighbor_prices, json_neighbor_centers, json_single_family, neighbor_house_record, metrics, cities, city_data, family_dist, family_demo, city_seasonality, neighbor_demo, neighbor_stats, nbhd_stats, sectors = load_files()
...
> *Click **see future** to enter your choices of attributes in the Dropdown list. Click **check history** to see recent sold houses.*
'''
            ...
            children = [
                ...
                html.Div([
                    ...
                    html.Div([
                        ...
                        dbc.Card([
                            dbc.CardHeader('Check history (leave blank if you do not have any preference)'),
                            ...
                            dbc.CardBody([ # Sale price min-max
                                ...
                                dbc.Row([
                                    dbc.Col(
                                        dcc.Dropdown(
                                            id = 'sale-price-min',
                                            options = [{'label': price_min, 'value': price_min} for price_min in [0, 100000, 200000, 300000, 400000]],
                                            #value = family_demo['SALE_PRICE'].min(),
                                            placeholder = 'min (USD)'
                                        ),
                                    ),
                                    dbc.Col(
                                        dcc.Dropdown(
                                            id = 'sale-price-max',
                                            options = [{'label': price_max, 'value': price_max} for price_max in [600000, 700000, 800000, 900000, 1000000]],
                                            #value = family_demo['SALE_PRICE'].max(),
                                            placeholder = 'max (USD)'
                                        ),
                                    ),
                                ]),
                            ...
                            dbc.CardBody([ # Sale price min-max
                                ...
                                dbc.Row([
                                    dbc.Col(
                                        dcc.Dropdown(
                                            id = 'sale-year-min',
                                            options = [{'label': year_min, 'value': year_min} for year_min in [2000, 2005, 2010, 2015]],
                                            #value = family_demo['SALE_YEAR'].min(),
                                    ...
                                    dbc.Col(
                                        dcc.Dropdown(
                                            id = 'sale-year-max',
                                            options = [{'label': year_max, 'value': year_max} for year_max in [2015, 2019]],
                                            #value = family_demo['SALE_YEAR'].max(),
                            ...
                            dbc.CardBody([ # above ground area min-max
                                ...
                                dbc.Row([
                                    dbc.Col(
                                        dcc.Dropdown(
                                            id = 'history-area-min',
                                            options = [{'label': area_min, 'value': area_min} for area_min in [500, 1000, 1500, 2000]],
                                            #value = 600,
                                    ...
                                    dbc.Col(
                                        dcc.Dropdown(
                                            id = 'history-area-max',
                                            options = [{'label': area_max, 'value': area_max} for area_max in [4500, 5000, 5500, 6000]],
                            ...
                            dbc.CardBody([ # lot size min-max
                                ...
                                dbc.Row([
                                    dbc.Col(
                                        dcc.Dropdown(
                                            id = 'hisotry-lot-min',
                                            options = [{'label': lot_min, 'value': lot_min} for lot_min in [3000, 4000, 5000]],
                                    ...
                                    dbc.Col(
                                        dcc.Dropdown(
                                            id = 'hisotry-lot-max',
                                            options = [{'label': lot_max, 'value': lot_max} for lot_max in [8000, 9000, 10000]],
                    ...
                    html.Div([
                        ...
                        html.Button('Download Table', id = 'save-table', n_clicks = 0, style = {'display': 'none'}),
                    ], id = 'house-map-chart', className = 'eight columns', style = {'margin-right': 30}),

                    html.Div(id = 'predict-table', style = {'display': 'none'}),
                    html.Div(id = 'save-table-complete', children = [dcc.ConfirmDialog(id = 'confirm', message = 'Prediction table is saved in Documents.')]),
...
def update_house_map(click1, click2):
    if click2 == 0:
        return [{'display': 'none'}]
    else:
...
# Remove old callback using Output('denver-house-map', 'figure')
...
def update_house_map(min_price, max_price, min_year, max_year, min_bed, max_bed, minarea, maxarea, minlot, maxlot, neighbor, story, nclicks):
    if nclicks == 0:
        return draw_house_base_map(json_neighbor, neighbor_house_record)
    else:
        family_demo_filtered = family_demo_filter(min_price, max_price, min_year, max_year, min_bed, max_bed, minarea, maxarea, minlot, maxlot, neighbor, story, family_demo)
        return update_house_base_map(json_single_family, family_demo_filtered)
...
def predict_house_price(neighbor, bed, bath, story, age, minarea, maxarea, minlot, maxlot, park, store, month, n_clicks, confirm_sel):
    if n_clicks == 1 and confirm_sel == 0 and neighbor is None:
        return [[dcc.Graph(
            ...
            figure = draw_house_base_map(json_neighbor, neighbor_house_record),
            style = {'height': '1000px'}
        ),html.Button('Download Table', id = 'save-table', n_clicks = 0, style = {'display': 'none'}),],'', {'display': 'block'}]

    if confirm_sel > 0:
        predict_tab = predict_house_xgb(neighbor, bed, bath, story, age, minarea, maxarea, minlot, maxlot, park, store, month, neighbor_stats, neighbor_demo)
        ...
        return [[
            ...
            html.Code('Highest Predicted Price at ${:,.0f} with {} stories, {} bedrooms, {} bathrooms, {:,.0f}sqft above ground area, {:,.0f}sqft lot zize, {:,.0f}sqft garden area, {:,.0f} meter to closest park, and {:,.0f} meter to closest food stores.'
            ...
            html.Code('Lowest Predicted Price at ${:,.0f} with {} stories, {} bedrooms, {} bathrooms, {:,.0f}sqft above ground area, {:,.0f}sqft lot zize, {:,.0f}sqft garden area, {:,.0f} meter to closest park, and {:,.0f} meter to closest food stores.'
            ...
            html.Button('Download Table', id = 'save-table', n_clicks = 0, style = {'margin-top': 20}),
        ...
        return [[dcc.Graph(
            ...
            figure = draw_house_base_map(json_neighbor, neighbor_house_record),
            style = {'height': '1000px'}
        ),html.Button('Download Table', id = 'save-table', n_clicks = 0, style = {'display': 'none'}),],'', {'display': 'none'}]
...
def save_predict_table(n_clicks, values):
    if n_clicks > 0:
        df = pd.read_json(values)
        filename = os.path.join(os.path.expanduser('~'),'Documents','denver-house-prediction.csv')
        df.to_csv(filename)
        return True
    else:
        return False
