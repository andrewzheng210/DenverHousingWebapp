                ...
                dbc.CardBody(
                    [
                        dcc.Markdown('''
                            ...
                            > With the processed data, we built an XGBoost model and compared with a baseline Lasso regressio model. 
            ...
            dbc.CardBody(
                [ 
                    ...
                    *[Li Li](https://www.linkedin.com/in/li-li-69bb60128/)*, *[Liuhui Zhao](https://www.linkedin.com/in/lhzhao/)*, and *[Zehua Zheng](https://www.linkedin.com/in/zehuazheng/)*.
                    ...
                    > We came from different disciplines, formed *Team Flying Pig*, and accomplished this project for Denver housing market analytics and visulization in Spring, 2020.
                    '''),
                ]
            ),
...
def draw_base_map(json_neighbor, neighbor_prices, neighbor_house_record, sel_nbhd):
    data=[{
            ...
            'marker': {'opacity': 0.7, 'line': {'color': 'lightgrey'}},
            'hovertemplate': "%{location} <br>Median Sale Price (2019): $%{z:,.f}",
            'colorbar':{
                'title': {'text': 'Neighborhood Median Sale Price 2019'}, 
                'x': 0},
...
# update house hunting map - new by Andrew
...
def combine_jsons():
    with open('JsonFiles/Combined_Stores.json') as json_file:
        stores=json.load(json_file)
    return stores

def draw_store_map(stores_json,sel_nbhd):
    stores=stores_json[sel_nbhd]["stores"]
    nbhd=stores_json[sel_nbhd]["center"]
    store_type={}
    store_data=[]
    for store in stores:
        if store["store type"] in store_type:
            store_type[store["store type"]]["lat"].append(store["cord"][1])
            store_type[store["store type"]]["lon"].append(store["cord"][0])
            store_type[store["store type"]]["text"].append(store["store name"])
        else:
            store_type[store["store type"]]={
                "lat":[store["cord"][1]],
                "lon":[store["cord"][0]],
                "text":[store["store name"]]
            }

    for s_type in store_type:
        store_data.append(
            go.Scattermapbox(
            lat=store_type[s_type]["lat"],
            lon=store_type[s_type]["lon"],
            text=store_type[s_type]["text"],
            hoverinfo='text',
            mode='markers',
            marker=dict(size=14),
            opacity=1.0,
    ...
    return {
        ...
        'layout': {
            'title': "Points of Interests in "+sel_nbhd,
            'height': 300,
            'width': 600,
            'margin': {"r":10, "t": 0, "l":10,"b":10},
            'hovermode': 'closest',
            ...
            'legend':{'x': 0, 'y': 1},
            'mapbox': {
                'center': {'lat': nbhd["cord"][1], 'lon': nbhd["cord"][0]},
                'zoom': 12,
