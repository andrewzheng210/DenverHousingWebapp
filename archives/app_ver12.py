from utils import load_files, draw_base_map, draw_house_base_map, update_house_base_map, predict_house_xgb, draw_family_base_map, write_about, load_markdowns, load_app_header, family_demo_filter,combine_jsons,draw_store_map
...
def reset_house_history(n_clicks):
    if n_clicks > 0:
        return (0, 0,
        500000, 1000000, 2010, 2019, None, None, 1000, 5000, 2000, 10400, None, None
        )
    else:
        raise PreventUpdate
...
# callbacks to display map of stores in nbhd
@app.callback(
    [Output('nbhd-stores', 'options'),
    Output('nbhd-stores', 'value')],
    [Input('neighbor-price-chart-list', 'value')]
)
def update_stores_choices(sel_val):
    if sel_val==None or len(sel_val)==0:
        raise PreventUpdate
    return [[{'label': nbhd_name, 'value': nbhd_name} for nbhd_name in sel_val],sel_val[0]]

@app.callback(
    Output('stores-map','figure'),
    [Input('nbhd-stores', 'value')]
)
def update_stores_map(sel_val):
    if sel_val==None or len(sel_val)==0:
        raise PreventUpdate
    return draw_store_map(stores_json,sel_val)
