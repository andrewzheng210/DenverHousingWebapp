...
json_neighbor, neighbor_prices, json_neighbor_centers, single_family, neighbor_house_record, metrics, cities, city_data, family_dist, family_demo, city_seasonality, nbhd_stats, sectors = load_files()
app.index_string = '''
    <!DOCTYPE html>
    <html>
        <head>
...
def show_choice(sel_val):
    ...
    return {'margin-top': 0, 'margin-right': 30,"display":"block"}

@app.callback(
    [Output('nbhd-table', 'columns'),
    Output('nbhd-table', 'data')],
    [Input('neighbor-price-chart-list', 'value'),
    Input('nbhd-info','value')]
)
# Inspired by https://dash.plotly.com/datatable
def print_table(sel_val1,sel_val2):
    if sel_val1==None or len(sel_val1)==0:
        raise PreventUpdate
    if sel_val2==None or len(sel_val2)==0:
        raise PreventUpdate
        ...
        return {
            ...
            'layout': {
                ...
                'xaxis': {
                    ...
                    'showgrid': False,
                    'ticks': 'outside',
                    'range': [min(neighbor_prices[met]), max(neighbor_prices[met])]
                    },
                'yaxis':{
                    'showline': True,
                    'linecolor': 'gray',
