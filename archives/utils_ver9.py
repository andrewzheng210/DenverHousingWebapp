...
def update_house_base_map(json_single_family, family_demo):
    return {
        'data': [{
                'type': "choroplethmapbox",
                'geojson': json_single_family,
                ...
                'name': family_demo['SALE_YEAR'],
                'featureidkey': 'properties.SCHEDNUM',
                'marker': {'opacity': 1.0, 'line': {'color': 'lightgrey'}},
                'hovertemplate': "Sales Price: %{z:.0f} <br> Sale Year: %{name:.0f} ",
                'showscale': False,
                'colorscale': 'Reds',
            }
            #,
            ...
            #    'name': '',
            #    'featureidkey': 'properties.NBHD_NAME',
            #    'marker': {'opacity': 0.3, 'line': {'color': 'lightgrey'}},
...
def family_demo_filter(min_price, max_price, min_year, max_year, min_bed, max_bed, minarea, maxarea, minlot, maxlot, neighbor, story, family_demo):
    all_true_filter = pd.Series([True for i in range (family_demo.shape[0])])

    if min_price is None:
        min_price_filter = all_true_filter
    else:
        min_price_filter = family_demo['SALE_PRICE'] >= min_price

    if max_price is None:
        max_price_filter = all_true_filter
    else:
        max_price_filter = family_demo['SALE_PRICE'] <= max_price

    if min_year is None:
        min_year_filter = all_true_filter
    else:
        min_year_filter = family_demo['SALE_YEAR'] >= min_year

    if max_year is None:
        max_year_filter = all_true_filter
    else:
        max_year_filter = family_demo['SALE_YEAR'] <= max_year

    if min_bed is None:
        min_bed_filter = all_true_filter
    else:
        min_bed_filter = family_demo['BED_RMS'] >= min_bed

    if max_bed is None:
        max_bed_filter = all_true_filter
    else:
        max_bed_filter = family_demo['BED_RMS'] <= max_bed

    if minarea is None:
        minarea_filter = all_true_filter
    else:
        minarea_filter = family_demo['AREA_ABG'] >= minarea

    if maxarea is None:
        maxarea_filter = all_true_filter
    else:
        maxarea_filter = family_demo['AREA_ABG'] <= maxarea

    if minlot is None:
        minlot_filter = all_true_filter
    else:
        minlot_filter = family_demo['LAND_SQFT'] >= minlot

    if maxlot is None:
        maxlot_filter = all_true_filter
    else:
        maxlot_filter = family_demo['LAND_SQFT'] <= maxlot

    if neighbor is None:
        neighbor_filter = all_true_filter
    else:
        neighbor_filter = family_demo['NBRHD_NAME'] == neighbor

    if story is None:
        story_filter = all_true_filter
    else:
        story_filter = family_demo['STORY'] == story


    return family_demo[min_price_filter & max_price_filter & min_year_filter & max_year_filter & min_bed_filter & max_bed_filter & minarea_filter & maxarea_filter & minlot_filter & maxlot_filter & neighbor_filter & story_filter]
